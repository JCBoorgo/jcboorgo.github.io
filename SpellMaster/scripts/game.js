...
function onLoad() {
  ...
  if (casts >= 5) {
    ...
    for (i = 0; i < selects.length; i++) {
      ...
      select.appendChild(opt);
			changeText("createSpellDescription", "The game dev hasn't made more spells yet, stay tuned!");
			document.getElementById("createSpellbtn").style.display = "none";
			document.getElementById("createSpellCost").style.display = "none";
    }
  }
  ...
  for (i = 1; i <= game.conjurationSpells.createCaster.timesCast; i++) {
	...
	if (game.conjurationSpells.createCaster.timesCast >= 4) {
    changeText("createCasterDescription", "You can't create more autocasters.");
...
function createSpell() {
  ...
  switch (game.conjurationSpells.createSpell.timesCast) {
    ...
    case 4:
      document.getElementById("focusMult").style.display = "inline-block";
      game.conjurationSpells.createSpell.cost = 1e100;
      for (i = 0; i < selects.length; i++) {
        select = selects[i];
        opt = document.createElement('option');
        opt.value = "focusMultSpell";
...
//Toggle: "N" for normal spell, "P" with power upg, "D" with duration, "C" with cost
function getSpellEfficiency(spell, toggle) {
  switch (toggle) {
    case "N":
      return spell.power * spell.duration / spell.cost;
    case "P":
      return (spell.power + spell.powerPos) * (spell.duration - spell.durationNeg) / spell.cost;
    case "D":
      return spell.power * (spell.duration + spell.durationPos) / (spell.cost + spell.costNeg);
    case "C":
      return spell.power * spell.duration / (spell.cost - spell.costPos);
  }
}
...
function openThanks() {
  window.open("thanks.html", "_newtab");
}
